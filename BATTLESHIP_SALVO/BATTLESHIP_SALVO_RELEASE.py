...
def start_screen():
    ...
    pygame.display.flip()
    pygame.mixer.music.load('marching.mp3')
...
#movement-----------------------------------------------------------------------
    ...
    def enemy_turn(self, ocean, ocean_2):     # this handles computer actions
        ...
        if ocean.Chk_tile(attack_coord) == True:
            self.hitlist.append(attack_coord)
            ...
            hit = False
            while hit == False:
                if attack_coord not in self.hitlist:
                    self.hitlist.append(attack_coord)
                    print 'they missed!'
                    empty = pygame.image.load("empty.png") # draw logo
                    rectempty = empty.get_rect()               
                    ...
                    time.sleep(1)
                    hit = True
                elif attack_coord in self.hitlist:
    ...
    def moves(self):
        ...
        shots = ocean.Remains()
        eshots = ocean_2.Remains()
        if self.turn == 0 or self.turn % 2 == 0: # player plays first
            self.player_turn(ocean, ocean_2)
            while shots > 1:
                self.victory(ocean, ocean_2) #checks for win or lose
